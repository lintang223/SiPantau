...
from scraper.browser_manager import safe_goto, is_blocked
...
async def _apply_stealth(page):
    ...
    if _STEALTH_AVAILABLE:
        try:
            ...
            print(f"      [STEALTH ERROR] Gagal apply stealth: {e}")
                ...
                for (let card of cards) {
                    try {
                        ...
                        if (link && !results.has(link)) {
                            ...
                            let priceEl  = card.querySelector("div[class*='urMOlDHH']") || card.querySelector("[data-testid='spnSRPProdPrice']");
                            ...
                            if (!price) {
                                let m = card.innerText.match(/Rp\s*[\d\.]+/i);
...
async def scrape_all_pages(
    ...
    is_cdp: bool = False,
    max_load_more: int = None,        # legacy — tidak lagi dipakai
    target_product_count: int = None, # jumlah produk BARU yang ingin diambil sesi ini
    harga_threshold: int = 0,        # harga minimum — produk di bawah ini di-skip
    ...
    MAX_SAFETY_SCROLL = 50  # batas scroll agar tidak infinite loop

    # Muat link produk yang sudah pernah di-scraping (untuk Resume)
    from scraper.utils import backup_load_done_links
    done_links = backup_load_done_links(keyword)
    seen_links = set(done_links)
    if done_links:
        print(f"   🔄 Resume: {len(done_links)} produk lama di-skip (sudah pernah di-scraping)")

    _target = target_product_count if target_product_count and target_product_count > 0 else None
    ...
    while True:
        load_count += 1
        # Hentikan jika target jumlah produk BARU sudah terpenuhi
        if _target and len(all_products) >= _target:
            print(f"   🎯 Target {_target} produk baru tercapai — lanjut ke detail scraping.")
            break
        # Safety: cegah infinite loop jika produk habis
        if load_count > MAX_SAFETY_SCROLL:
            print(f"   ⚠️  Batas safety {MAX_SAFETY_SCROLL} scroll tercapai — berhenti.")
            break
        ...
        # ── Filter berdasarkan harga minimum (threshold) ─────────────────────
        ...
        # Potong jika melebihi target
        if _target:
            sisa = _target - len(all_products)
            new_prods = new_prods[:sisa]

        all_products.extend(new_prods)
        new_this  = len(all_products) - before

        target_info = f"/{_target}" if _target else ""
        print(f"      ✅ +{new_this} produk baru | Sesi ini: {len(all_products)}{target_info}")
                ...
                if throttle_count >= THROTTLE_LIMIT:
                    ...
                    if context_ref and browser:
                        if is_cdp:
                            ...
                            from scraper.browser_manager import create_context
