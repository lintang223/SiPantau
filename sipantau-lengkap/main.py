...
def init_db():
    ...
    # Tambah kolom password_plain kalau belum ada
    cur.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (
    ...
    # Tambah kolom username kalau belum ada (migrasi dari versi lama)
    cur.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM information_schema.columns
                WHERE table_name='hasil_scraping' AND column_name='username'
            ) THEN
                ALTER TABLE hasil_scraping ADD COLUMN username TEXT;
            END IF;
        END$$;
    """)

    cur.execute("""
        CREATE TABLE IF NOT EXISTS riwayat_session (
            id          SERIAL PRIMARY KEY,
            session_id  TEXT,
            username    TEXT,
            keyword     TEXT,
    ...
    # Tambah kolom username di riwayat kalau belum ada
    cur.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM information_schema.columns
                WHERE table_name='riwayat_session' AND column_name='username'
            ) THEN
                ALTER TABLE riwayat_session ADD COLUMN username TEXT;
            END IF;
        END$$;
    """)
...
def get_stats(username: str = ""):
    conn = get_conn(); cur = conn.cursor()
    if username:
        cur.execute("SELECT COUNT(*) FROM hasil_scraping WHERE username = %s", (username,))
        total = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM hasil_scraping WHERE username = %s AND LOWER(platform) = 'tokopedia'", (username,))
        tokped = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM hasil_scraping WHERE username = %s AND LOWER(platform) = 'shopee'", (username,))
        shopee = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM hasil_scraping WHERE username = %s AND LOWER(platform) = 'lazada'", (username,))
        lazada = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM riwayat_session WHERE username = %s", (username,))
