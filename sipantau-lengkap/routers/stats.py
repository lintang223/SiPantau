...
def get_stats(
    ...
    with get_conn() as conn:
        ...
        if divisi:
            ...
            total  = count("SELECT COUNT(*) FROM hasil_scraping WHERE username=?", (username,))
            tokped = count("SELECT COUNT(*) FROM hasil_scraping WHERE username=? AND LOWER(platform)='tokopedia'", (username,))
            ekspor = count("SELECT COUNT(*) FROM riwayat_session WHERE username=?", (username,))
        else:
